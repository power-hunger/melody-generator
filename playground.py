...
def main():
    ...
    oiter = stream.iterator.OffsetIterator(parts[1])
    for groupedElements in oiter:
        if len(groupedElements) >= 2:
            print(groupedElements, float(groupedElements[0].quarterLength),
                  float(groupedElements[1].quarterLength),
                  float(groupedElements[0].offset),
                  float(groupedElements[1].offset))
        else:
            print(groupedElements, float(groupedElements[0].quarterLength), float(groupedElements[0].offset))
...
def create_midi():
    ...
    offset = 0
    output_notes = []
    prediction_output = ['F#2', "Rest", "Rest", 'F#2', "Rest", "Rest", "Rest", 'F#2', "Rest", 'F#2', "Rest", 'F#2', "Rest",
                         "Rest", "Rest", 'B1', "Rest", "Rest", "Rest", "Rest", "Rest", "Rest", 'B1', "Rest", "Rest",
                         "Rest", "Rest", "Rest", "Rest", "Rest", 'B1', "Rest", "Rest", "Rest", "Rest", "Rest", "Rest",
                         "Rest", 'B', "Rest", "Rest", "Rest", "Rest", "Rest", "E2 E2", "Rest", "E-4 F3 C2"]
    # create note and chord objects based on the values generated by the model
    output_notes.append(key.Key('E-'))
    for pattern in prediction_output:
        if pattern == "Rest":
            ...
            new_chord = chord.Chord(['E2', 'E2'])
            new_chord.offset = offset
            new_chord.storedInstrument = instrument.Piano()
            output_notes.append(new_chord)
        elif pattern == "E-4 F3 C2":
            new_chord = chord.Chord(['E-4', 'F3', 'C2'])
            new_chord.offset = offset
            new_chord.storedInstrument = instrument.Piano()
            output_notes.append(new_chord)
        else:
